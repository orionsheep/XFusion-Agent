...
test('agent-first home and secondary workspace show PDF-facing capabilities', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'XFusion Agent' })).toBeVisible({ timeout: 15_000 })
  await expect(page.locator('.agent-stage__brand-orb')).toBeVisible({ timeout: 15_000 })
  await expect(page.getByPlaceholder(/尽管问 XFusion Agent/)).toBeVisible({ timeout: 15_000 })
  await expect(page.locator('.workspace-overlay')).toHaveCount(0)
  await expect(page.locator('.kimi-floating-controls').getByRole('button', { name: '工作区' })).toBeVisible({ timeout: 15_000 })
  await expect(page.locator('.kimi-floating-controls').getByRole('button', { name: '会话设置' })).toBeVisible({ timeout: 15_000 })
  await page.locator('.topbar__chip--model').click()
  await expect(page.getByText('模型切换', { exact: true })).toBeVisible({ timeout: 15_000 })
  await page.keyboard.press('Escape')
  await page.locator('.topbar__chip--hosts').click()
  await expect(page.getByText('目标主机', { exact: true })).toBeVisible({ timeout: 15_000 })
  await page.keyboard.press('Escape')
  await page.locator('.kimi-floating-controls').getByRole('button', { name: '工作区' }).click()
  await expect(page.getByRole('heading', { name: '工作区' })).toBeVisible({ timeout: 15_000 })
  await page.locator('.workspace-launcher__item').filter({ hasText: '总览' }).click()
